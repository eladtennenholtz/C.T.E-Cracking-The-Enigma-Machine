...
public class AgentsMainController implements Initializable, Closeable {
    ...
    private void initAllTheThreadsForContest() {
    ...
    private void startContest() throws IOException, JAXBException, InterruptedException {
        ...
        //InputStream source = new ByteArrayInputStream(response.body().string().getBytes((StandardCharsets.UTF_8)));
            ...
            HttpClientUtilAgents.runAsync(finalUrl, new Callback() {
                ...
                public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                    if (response.code() != 200) {
                        ...
                        try {
                            //Gson jsonNew=new Gson();
                            //tasks=jsonNew.fromJson(response.body().charStream(),new TypeToken<List<TaskDto>>(){}.getType());
                            jsonElement = jsonParser.parse(response.body().string());
                        ...
                        // Thread newThreadForThis=new Thread(()->{
                        Platform.runLater(() -> {
                            totalTasksPulledLabel.setText(String.valueOf(sumOfAllTaskedPulled));
                            // int taskTemp = Integer.parseInt(totalTasksPulledLabel.getText());
                            //taskTemp += taskForRunning;
                            //totalTasksPulledLabel.setText(String.valueOf(taskTemp));
                        });
                        //});newThreadForThis.start();
                        ...
                        for (int i = 0; i < taskForRunning; i++) {
                            //כאן
                            String starting = jsonElement.getAsJsonArray().get(i).getAsJsonObject().get("startingPosition").toString().substring(1, jsonElement.getAsJsonArray().get(i).getAsJsonObject().get("startingPosition").toString().length() - 1);
                            String startingPositionStr[] = starting.split(",");
                            //String startingPositionStr[]=new String[usedRotorsSize];
                            ArrayList<Integer> startingPosition = new ArrayList<>();
                            //startingPosition=(tasks.get(0).getStartingPosition());

                            //startingPosition=tasks.get(1).getStartingPosition();
                            //for(int n=tasks.get(0).getStartingPosition().size()-1;n>=0;n--){
                            //  startingPosition.add(tasks.get(0).getStartingPosition().get(n));
                            //}
                            for (int j = 0; j < startingPositionStr.length; j++) {

                                startingPosition.add(Integer.parseInt((startingPositionStr[j])));
                            }


                            String rotorsId="";
                            try {
                                rotorsId = jsonElement.getAsJsonArray().get(i).getAsJsonObject().get("rotorsId").toString().substring(1, jsonElement.getAsJsonArray().get(i).getAsJsonObject().get("rotorsId").toString().length() - 1);
                            }catch (IllegalStateException e){
                                Platform.runLater(()->{
                                    tasksInQueueLabel.setText("0");
                                });
                            ...
                            String strRotors[] = rotorsId.split(",");


                            String reflectorId="";
                            try {
                                reflectorId = jsonElement.getAsJsonArray().get(i).getAsJsonObject().get("reflectorId").toString();
                            }catch (IllegalStateException e){
                                Platform.runLater(()->{
                                    tasksInQueueLabel.setText("0");
                                });
                            ...
                            machine.addReflector(Integer.parseInt(reflectorId), enigmaMachine.allReflectors.get(Integer.parseInt(reflectorId)).getReflectorMapping());
                            // machine.addReflector(tasks.get(0).getReflectorId(),enigmaMachine.allReflectors.get(tasks.get(0).getReflectorId()).getReflectorMapping());
                            ...
                            int t=0;
                            for (String str : strRotors) {
                                usedRotors[t] = Integer.parseInt(str);
                                t++;
                            }
