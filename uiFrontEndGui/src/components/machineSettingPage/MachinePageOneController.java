package components.machineSettingPage;
...
public class MachinePageOneController implements Initializable {

    private MainController mainController;
    @FXML private TextArea machineDetailsText;
    @FXML private TextArea currentMachineConfiguration;
    ...
    public String reflectorToCheck;



    private ArrayList<Integer>allRotors;
    private ArrayList<Rotor>allRotorsReal;
    private ArrayList<Rotor>allRotorsLive;
    private ArrayList<String>allReflectors;
    private ArrayList<Reflector> allReflectorsReal;
    ...
    public MachinePageOneController(){
        machineDetailsTextProperty=new SimpleStringProperty();
        currentMachineConfigurationProperty=new SimpleStringProperty();
    ...
    public void setRandomCodeListener(ActionEvent actionEvent) {
        this.mainController.setRandomCodeListenerInChildHappened();
    }
    public void setRandomCodeInTextField(DataFromEngineToUi resultedData){
        ...
        this.mainController.setIsSet(true);
    }


    public void setDetailsInChildMachinePageOne(CurrentMachineDetails machineDetails) {
        //if(this.allRotors==null){
            ...
            counter=machineDetails.getUsedRotorsSize();
            setCodeManuallyButtonProperty.set(false);

            countRotors=machineDetails.getUsedRotorsSize();

            originalCountRotors=machineDetails.getUsedRotorsSize();
            countReflectors=1;
            countPlugs=machineDetails.getAlphabet().length()/2;
            countPlugsReal=machineDetails.getAlphabet().length()/2;
        ...
        this.allRotorsLive=new ArrayList<>(machineDetails.getAllRotors().size());
        this.allReflectorsReal=new ArrayList<>();
        this.plugOneChoiceBoxSet=new HashSet<>();
        this.plugTwoChoiceBoxSet=new HashSet<>();
        for(int i=0;i<machineDetails.getAllRotors().size();i++){
            allRotors.add(machineDetails.getAllRotors().get(i+1).getRotorId());
            allRotorsReal.add(machineDetails.getAllRotors().get(i+1));
        }
        rotorsChoiceBox.getItems().addAll(allRotors);
        for(int i=0;i<machineDetails.getAllReflectors().size();i++){
            allReflectors.add(convertReflectorsFromIntToString(machineDetails.getAllReflectors().get(i+1).getReflectorId()));
            allReflectorsReal.add(machineDetails.getAllReflectors().get(i+1));
        }
        reflectorChoiceBox.getItems().addAll(allReflectors);
        for(int i=0;i<machineDetails.getAlphabet().length();i++){
            plugBoardAlphabet.add(String.valueOf(machineDetails.getAlphabet().charAt(i)));
        }
        plugsOneChoiceBox.getItems().addAll(plugBoardAlphabet);
        plugsTwoChoiceBox.getItems().addAll(plugBoardAlphabet);

        //startingPositionChoiceBox.getItems().addAll(plugBoardAlphabet);
    ...
    public void setRotorsChoiceBox(ActionEvent actionEvent){
...
//            System.out.println(startingPositionChoiceBox.getValue());
    ...
    public void setPositionChoiceBox(ActionEvent actionEvent) {
    ...
    public void setCodeManuallyListener(ActionEvent actionEvent) {
        ...
        if(plugBoardShowingArea.getText()==null){
            ...
            String answer=this.mainController.setCodeManuallyPop(inputs);
...
//onChanged method
        ...
        int j=0;
        ArrayList<Rotor>tmp=new ArrayList<>();
    ...
    public Rotor duplicate(Rotor rotor){
        return new Rotor(rotor.getRotorId(),rotor.getNotchPosition(),rotor.getAmountOfEntriesInRotor(),rotor.forwardEntry,rotor.backwardEntry);
    }
...
//onChanged method
        ...
        int j=0;
        ArrayList<Reflector>tmp=new ArrayList<>();
        for(int k=0;k<allReflectorsReal.size();k++){
            tmp.add(duplicate(allReflectorsReal.get(k)));
        }
        Reflector r=tmp.get(convertReflectorsFromStringToInt(reflectorChoiceBox.getValue())-1);
    ...
    public Reflector duplicate(Reflector reflector){

        HashMap<Integer,Integer>newHashMap=new HashMap<>();
        newHashMap.putAll(reflector.getReflectorMapping());
        return new Reflector(reflector.getReflectorId(),newHashMap);
    }
